...
def get_projects():
    """Get user's projects"""
    projects = project_service.get_user_projects(GUEST_USER_ID)
...
def get_project(project_id):
    """Get project details"""
    project_data = project_service.get_project_dashboard_data(project_id, GUEST_USER_ID)
    if not project_data:
...
def get_project_tasks(project_id):
    ...
    tasks = task_service.get_project_tasks(project_id, GUEST_USER_ID)
    ...
    try:
        if 'texts' in data:  # Bulk create
            tasks = task_service.bulk_create_tasks(
                project_id, data['texts'], 1
            )
            return jsonify([t.to_dict() for t in tasks]), 201
        else:  # Single task
            task = task_service.create_task(
                project_id, data.get('text'), 1
            )
...
def get_task_annotations(task_id):
    ...
    annotations = annotation_service.get_task_annotations(task_id, GUEST_USER_ID)
...
def get_project_labels(project_id):
    """Get project labels"""
    labels = label_service.get_project_labels(project_id, GUEST_USER_ID)
...
def create_label(project_id):
    ...
    try:
        label = label_service.create_label(
            project_id, data.get('value'), 1, **data
        )
...
def import_data(project_id):
    ...
    try:
        result = data_import_service.import_jsonl_data(
            project_id, data.get('jsonl_data'), 1
        )
...
def get_statistics():
    ...
    if project_id:
        ...
        project_stats = project_service.get_project_dashboard_data(project_id, GUEST_USER_ID)
        if not project_stats:
            return jsonify({'error': 'Project not found'}), 404
        
        statistics = project_stats.get('statistics', {})
        return jsonify({
            'project_id': project_id,
            'total_annotations': statistics.get('total_annotations', 0),
            'labels_distribution': statistics.get('label_distribution', {}),
            'completion_rate': statistics.get('completion_percentage', 0.0),
            'total_tasks': statistics.get('total_tasks', 0),
            'completed_tasks': statistics.get('completed_tasks', 0)
        })
    else:
        ...
        return jsonify({
            ...
            'labels_distribution': {},
            'completion_rate': 0.0,
            'message': 'Use ?project_id=N for project-specific statistics'
        })
...
def export_task(task_id):
    ...
    try:
        task = task_service.get_task_by_id(task_id, GUEST_USER_ID)
        if not task:
...
def get_label(project_id, label_id):
    """Get specific label"""
    try:
        label = label_service.get_label_by_id(label_id, GUEST_USER_ID)
        if not label:
...
def delete_label(project_id, label_id):
    """Delete label"""
    try:
        success = label_service.delete_label(label_id, GUEST_USER_ID)
        if not success:
            return jsonify({'error': 'Label not found'}), 404
